    ...
    private void Awake() {
        for(int i = 0; i < lenY; i++){
            ...
            for(int j = 0; j < lenX; j++){
                ...
                _instantiatedObj = Instantiate(gridElement, new Vector3(transform.position.x + j + (j * (spaceX -(1-gridElement.transform.localScale.x))), transform.position.y + i + (i * (spaceY - (1 - gridElement.transform.localScale.y))), transform.position.z), Quaternion.Euler(0,0,45));
